...
const store = {
    _listeners: new Set(), _dispatch(prevState) {
    ...
    state: {
        ...
        menuOpen: false,
        openHelpTopic: null,
        fullscreen: isFullscreen(), // Note that config values used for <select>s must be strings, unless manually converting values to strings
...
const helpContent = {
    shellType: {
        header: '烟花类型', body: '选择将要发射的烟花类型。 选择“随机”是一个不错的选择！'
    }, shellSize: {
        header: '烟花大小', body: '烟花的大小：以真实的烟花炮弹尺寸为模型，较大的炮弹具有更大的爆发力，带有更多的星星，有时效果更复杂。 但是，较大的尺寸还需要更多的处理能力，并且可能会导致延迟。'
    }, quality: {
        header: '画质', body: '整体图形质量。 如果动画运行不流畅，请尝试降低质量。 高质量会大大增加产生的火花数量，并可能导致滞后。'
    }, skyLighting: {
        header: '天空照明', body: '烟花爆炸时照亮背景。 如果背景在屏幕上看起来太亮，请尝试将其设置为“暗淡”或“无”。'
    }, scaleFactor: {
        header: '规模', body: '允许缩放所有烟花的大小，实质上使您靠近或远离。 对于较大的烟花类型，可以稍微减小比例，尤其是在手机或平板电脑上。'
    }, autoLaunch: {
        header: '自动发射', body: '启用此选项即可自动启动烟花序列，取消勾选你就可以手动控制烟花的发射'
    }, finaleMode: {
        header: '结局模式', body: '发射强烈的烟花。 可能会导致延迟。 需要同时启用“自动发射”。'
    }, hideControls: {
        header: '隐藏控制器', body: '隐藏屏幕顶部的半透明控件。 对于屏幕截图很有用，或者只是更无缝的体验。 隐藏后，您仍然可以点击右上角以重新打开此菜单。'
    }, fullscreen: {
        header: '全屏', body: '切换全屏模式。'
    }, longExposure: {
        header: '打开快门', body: '实验效果可以保留长时间的光线，类似于保持相机快门打开。'
    }
};
...
function getConfigFromDOM() {
    return {
        ...
        hideControls: appNodes.hideControls.checked, // Store value as number.
...
const crysanthemumShell = (size = 1) => {
    ...
    const secondColor = singleColor && (Math.random() < 0.2 || color === COLOR.White) ? pistilColor || randomColor({
        notColor: color, limitWhite: true
    }) : null;
...
const horsetailShell = (size = 1) => {
    ...
    return {
        ...
        glitter: 'medium',
        glitterColor: Math.random() < 0.5 ? whiteOrGold() : color, // Add strobe effect to white horsetails, to make them more interesting
...
function init() {
    ...
    setOptionsForSelect(appNodes.quality, [{label: '低', value: QUALITY_LOW}, {
        label: '正常',
        value: QUALITY_NORMAL
    }, {label: '高', value: QUALITY_HIGH}]);

    setOptionsForSelect(appNodes.skyLighting, [{label: '无', value: SKY_LIGHT_NONE}, {
        label: '暗淡',
        value: SKY_LIGHT_DIM
    }, {label: '正常', value: SKY_LIGHT_NORMAL}]);

    // 0.9 is mobile default
    setOptionsForSelect(appNodes.scaleFactor, [0.5, 0.62, 0.75, 0.9, 1.0, 1.5, 2.0]
...
function getRandomShellSize() {
    ...
    return {
        size, x: fitShellPositionInBoundsH(x), height: fitShellPositionInBoundsV(height)
    };
}
...
class Shell {
    ...
    launch(position, launchHeight) {
        ...
        const comet = this.comet = Star.add(launchX, launchY, typeof this.color === 'string' && this.color !== 'random' ? this.color : COLOR.White, Math.PI, launchVelocity * (this.horsetail ? 1.2 : 1), // Hang time is derived linearly from Vi; exact number came from testing
    ...
    burst(x, y) {
        ...
        const starFactory = (angle, speedMult) => {
            ...
            const star = Star.add(x, y, color || randomColor(), angle, speedMult * speed, // add minor variation to star life
                this.starLife + Math.random() * this.starLife * this.starLifeVariation, this.horsetail ? this.comet && this.comet.speedX : 0, this.horsetail ? this.comet && this.comet.speedY : -standardInitialSpeed);
        ...
        if (typeof this.color === 'string') {
            ...
            if (this.ring) {
                ...
                createParticleArc(0, PI_2, this.starCount, 0, angle => {
                    ...
                    const star = Star.add(x, y, color, newAngle, // apply near cubic falloff to speed (places more particles towards outside)
                        newSpeed,//speed,
...
const soundManager = {
    ...
    sources: {
        lift: {
            volume: 1, playbackRateMin: 0.85, playbackRateMax: 0.95, fileNames: [

                'lift1.mp3', 'lift2.mp3', 'lift3.mp3']
        }, burst: {
            volume: 1, playbackRateMin: 0.8, playbackRateMax: 0.9, fileNames: ['burst1.mp3', 'burst2.mp3']
        }, burstSmall: {
            volume: 0.25, playbackRateMin: 0.8, playbackRateMax: 1, fileNames: ['burst-sm-1.mp3', 'burst-sm-2.mp3']
        }, crackle: {
            volume: 0.2, playbackRateMin: 1, playbackRateMax: 1, fileNames: ['crackle1.mp3']
        }, crackleSmall: {
            volume: 0.3, playbackRateMin: 1, playbackRateMax: 1, fileNames: ['crackle-sm-1.mp3']
        }
    },
